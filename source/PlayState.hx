...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'daisies':
				dialogue = CoolUtil.coolTextFile(Paths.txt('daisies/daisiesDialogue'));
			case 'error':
				dialogue = CoolUtil.coolTextFile(Paths.txt('error/errorDialogue'));
			case 'hard-drive':
				dialogue = CoolUtil.coolTextFile(Paths.txt('hard-drive/hard-driveDialogue'));	
			case 'spam':
				dialogue = CoolUtil.coolTextFile(Paths.txt('spam/spamDialogue'));	
		}
		...
		switch(stageCheck)
		{
	  		...
	  		case 'desktop':
			{
				curStage = 'desktop';
					...
					var bg:FlxSprite = new FlxSprite(-100).loadGraphic(Paths.image('desktop/sky', 'shared'));
					bg.scrollFactor.set(0.1, 0.1);
					add(bg);

					var city:FlxSprite = new FlxSprite(-10).loadGraphic(Paths.image('desktop/city', 'shared'));
					city.scrollFactor.set(0.3, 0.3);
					city.setGraphicSize(Std.int(city.width * 0.85));
					city.updateHitbox();
					add(city);
					...
					for (i in 0...5)
					{
							var light:FlxSprite = new FlxSprite(city.x).loadGraphic(Paths.image('desktop/win' + i, 'shared'));
							light.scrollFactor.set(0.3, 0.3);
							light.visible = false;
							light.setGraphicSize(Std.int(light.width * 0.85));
							light.updateHitbox();
					...
					var streetBehind:FlxSprite = new FlxSprite(-40, 50).loadGraphic(Paths.image('desktop/behindTrain','shared'));
					add(streetBehind);

					phillyTrain = new FlxSprite(2000, 360).loadGraphic(Paths.image('desktop/train','shared'));
					if(FlxG.save.data.distractions){
						add(phillyTrain);
					}
					...
					var street:FlxSprite = new FlxSprite(-40, streetBehind.y).loadGraphic(Paths.image('desktop/street','shared'));
					add(street);
			}
			case 'stage':
				{
